...
当前正在讨论的首选方向是暂命名为 **GamePilot** 的游戏智能测试 Agent 平台：由 Agent 理解游戏规则或设计文档，规划测试路径，通过工具操作一个可控的游戏测试环境，校验游戏状态，发现异常，收集证据，生成缺陷报告，并将缺陷沉淀为回归测试。

为控制范围，游戏部分倾向于采用“最小可控游戏测试靶场”，而不是开发完整商业小游戏。可从无界面的回合制游戏状态机开始，随后按需要增加简单网页 UI、Playwright 操作和视觉测试能力。

该方向目前是**候选主方向，尚未完成正式立项和范围冻结**。在项目所有者确认之前，不应擅自创建大规模代码骨架或锁定具体框架。
...
- 制定阶段路线和学习计划；
- 初始化代码仓库和工程骨架；
- 编写并下发第一张 Claude Code 实施任务单。

## 11. 下一决策点

在开始大规模编码之前，需要依次确认：

1. 是否正式采用 GamePilot 游戏智能测试 Agent 方向；
2. 第一阶段使用无界面状态机、简单网页游戏，还是改造现有开源游戏；
3. MVP 首个测试域是战斗、商店/背包、任务奖励还是存档；
4. MVP 的验收指标和预植入缺陷集合；
5. 第一阶段数据库与 Docker 的引入深度；
6. 仓库名称、许可证和公开策略。
