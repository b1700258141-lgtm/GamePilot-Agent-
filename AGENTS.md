...
- 完成本阶段对应的原理学习和代码复盘；
- 确定第二阶段数据库与 Docker 的详细范围；
- 最终确认项目公开名称、许可证和完整 README 展示策略。

## 11. 下一决策点

当前第一阶段不再增加功能范围。完成学习复盘后，需要依次确认：

1. 第二阶段是否同时引入 PostgreSQL 与 Docker Compose，或继续拆分学习；
2. 战斗会话、事件历史和测试运行记录的数据模型；
3. 首批预植入缺陷及其验收指标；
4. Agent 首次接入采用 LangGraph 工具调用还是先实现 MCP 工具层；
5. 项目公开名称、许可证和仓库展示策略。
